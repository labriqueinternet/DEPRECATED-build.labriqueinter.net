...
apt-get remove -y --force-yes --purge linux-image-4* flash-kernel u-boot-tools u-boot-sunxi
...
apt-get clean
apt-get update
apt-get install -y --force-yes linux-image-armmp flash-kernel u-boot-sunxi u-boot-tools
